...
def space_bhe(image:np.ndarray,space:LogSpace,X_min=0,X_max=255)->np.ndarray:
    ...
    leq_cumulative_distribution=np.histogram(image,bins=median+1,range=(X_min+1,median))[0]/leq_median
    for i in range(1,len(leq_cumulative_distribution)):
        leq_cumulative_distribution[i]+=leq_cumulative_distribution[i-1]

    g_cumulative_distribution=np.histogram(image,bins=X_max+1-median,range=(median+1,X_max+1))[0]/g_median
    for i in range(1,len(g_cumulative_distribution)):
        g_cumulative_distribution[i]+=g_cumulative_distribution[i-1]
    ...
    for i in range(new_image.shape[0]):
        for j in range(new_image.shape[1]):
            if image[i,j]<=median:
                new_image[i,j]=space.sum(X_min,space.s_mul(space.sub(median,X_min),leq_cumulative_distribution[image[i,j]]))
            else:
                new_image[i,j]=space.sum(median,space.s_mul(space.sub(X_max,median),g_cumulative_distribution[image[i,j]-median-1]))
