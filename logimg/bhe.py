...
def space_bhe(image:np.ndarray,space:LogSpace,X_min=0,X_max=255)->np.ndarray:
    ...
    leq_cumulative_distribution=np.histogram(neg_image,bins=median+1,range=(X_min,median))[0]/leq_median
    for i in range(1,len(leq_cumulative_distribution)):
        leq_cumulative_distribution[i]+=leq_cumulative_distribution[i-1]

    g_cumulative_distribution=np.histogram(neg_image,bins=X_max-median,range=(median+1,X_max))[0]/g_median
    for i in range(1,len(g_cumulative_distribution)):
        g_cumulative_distribution[i]+=g_cumulative_distribution[i-1]
...
s_b=space_bhe(m,LIPSpace())
plt.imshow(s_b,cmap='gray',interpolation='nearest')
plt.show()

plt.hist(m.ravel(), bins=256, range=(0, 255), fc='k', ec='k')
plt.show()

plt.hist(b.ravel(), bins=256, range=(0, 255), fc='k', ec='k')
plt.show()

plt.hist(s_b.ravel(), bins=256, range=(0, 255), fc='k', ec='k')
plt.show()