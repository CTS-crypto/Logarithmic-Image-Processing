...
class PLIPImage(LogImage):
    ...
    def __add__(self,other:'PLIPImage')->'PLIPImage':
        add_image=PLIPImage(np.array([[1]]),self.M)
        if isinstance(other,PLIPImage):
    ...
    def __sub__(self,other:'PLIPImage')->'PLIPImage':
        add_image=PLIPImage(np.array([[1]]),self.M)
        if isinstance(other,PLIPImage):
            add_image.image=self.image-other.image
        elif isinstance(other,int) or isinstance(other,float):
    ...
    def __mul__(self,scalar)->'PLIPImage':
...
class PLIPSpace(LogSpace):
    ...
    def sum(self,f:np.ndarray,g:np.ndarray)->np.ndarray:
        ...
        return f_aux+g_aux-(f_aux*g_aux)/1026

    def sub(self,f:np.ndarray,g:np.ndarray)->np.ndarray:
        f_aux=np.array(f.tolist())
        g_aux=np.array(g.tolist())
        return (f_aux-g_aux)/(1-g_aux/1026)
